...
.gantt-task-delete{{width:26px;height:26px;display:inline-flex;align-items:center;justify-content:center;margin-left:auto;flex:0 0 auto;border:1px solid #fecaca;background:#fff;color:#b91c1c;border-radius:6px;font-size:18px;line-height:1;cursor:pointer;}}
.gantt-task-delete:hover{{background:#fef2f2;}} .gantt-task-delete:disabled{{cursor:wait;opacity:.6;}}
...
.todo-row-actions button[data-delete]{{background:#fef2f2;color:#b91c1c;}}
.todo-row-actions button:disabled{{cursor:wait;opacity:.6;}}
...
async function deleteTodoRemote(id) {{
  ...
  if (!Array.isArray(deleted) || !deleted.some(row=>String(row.id) === String(id))) {{
    throw new Error('태스크 삭제 결과를 확인할 수 없습니다.');
  }}
}}
async function deleteTodoItem(id, button) {{
  const item = todoItems.find(todo=>String(todo.id) === String(id));
  if (!item) return;
  if (!confirm(`'${{item.task}}' 태스크만 삭제하시겠습니까?\n프로젝트의 다른 태스크는 유지됩니다.`)) return;
  if (button) button.disabled = true;
  try {{
    await deleteTodoRemote(item.id);
  }} catch(e) {{
    console.warn('Supabase todo delete failed. Local delete applied.', e);
  }}
  todoItems = todoItems.filter(todo=>String(todo.id) !== String(item.id));
  saveTodoItems();
  renderTodo();
}}
...
function renderTodoGantt() {{
      ...
      ${{items.map(item=>{{
        ...
        return `<div class="gantt-row" data-open-todo="${{item.id}}">
          <div class="gantt-task-name">
            <span class="gantt-dot ${{todoStatusClass(item.status)}}"></span>
            <span class="gantt-task-label" title="${{item.memo || ''}}">${{item.task}}</span>
            <button class="gantt-task-delete" type="button" data-delete-todo="${{item.id}}" title="이 태스크만 삭제" aria-label="이 태스크만 삭제">&times;</button>
          </div>
  ...
  el.querySelectorAll('[data-delete-todo]').forEach(button=>button.addEventListener('click',async event=>{{
    event.stopPropagation();
    await deleteTodoItem(button.dataset.deleteTodo, button);
  }}));
...
function renderTodoList() {{
  ...
  body.querySelectorAll('[data-delete]').forEach(btn=>btn.addEventListener('click',async event=>{{
    event.stopPropagation();
    await deleteTodoItem(btn.dataset.delete, btn);
  }}));
}}
